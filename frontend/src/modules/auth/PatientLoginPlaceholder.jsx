...
export default function PatientLogin() {
  ...
  return (
    <AuthShell icon={User} title="Patient sign in" subtitle="Appointments and nearby care" accent="blue">
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label htmlFor="patient-email" className="care-label">Email address</label>
        ...
        <div>
          <label htmlFor="patient-password" className="care-label">Password</label>
          <div className="relative">
            <Lock className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-care-muted" />
            <input id="patient-password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required placeholder="Enter your password" value={password} onChange={(event) => setPassword(event.target.value)} className="care-input pl-10 pr-11" />
