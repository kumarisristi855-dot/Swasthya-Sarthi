...
export default function AdminLogin() {
  ...
  return (
    <AuthShell icon={Building2} title={t('hospitalSignIn')} subtitle={t('hospitalPortalSubtitle')} accent="amber">
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label htmlFor="admin-email" className="care-label">{t('emailAddress')}</label>
        ...
        <div>
          <label htmlFor="admin-password" className="care-label">{t('password')}</label>
          <div className="relative">
            <Lock className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-care-muted" />
            <input id="admin-password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required placeholder={t('passwordPlaceholder')} value={password} onChange={(event) => setPassword(event.target.value)} className="care-input pl-10 pr-11" />
      ...
      <p className="mt-6 text-center text-xs leading-5 care-muted">{t('hospitalProvisioned')}</p>
    </AuthShell>
  );
}
