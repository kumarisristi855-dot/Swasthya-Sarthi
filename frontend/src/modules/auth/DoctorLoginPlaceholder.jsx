...
export default function DoctorLogin() {
  ...
  return (
    <AuthShell icon={Stethoscope} title="Doctor sign in" subtitle="Clinic appointments and availability" accent="teal">
      {error && (
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label htmlFor="doctor-email" className="care-label">Email address</label>
        ...
        <div>
          <label htmlFor="doctor-password" className="care-label">Password</label>
          <div className="relative">
            <Lock className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-care-muted" />
            <input id="doctor-password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" required placeholder="Enter your password" value={password} onChange={(event) => setPassword(event.target.value)} className="care-input pl-10 pr-11" />
